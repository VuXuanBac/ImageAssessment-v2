...
def summarize(
    *, file=None, data=None, metrics: dict, corrs: dict, output_file: str = None
):
    if file:
        data = pd.read_csv(file, header=None)
    data_sum = data.apply(_summarize, axis=1, metrics=metrics)
    print(
        f"{'Accuracy':<13}: {binary_accuracy(data_sum['mean'], data_sum['gt_mean']) * 100:.2f}%"
    )
    data_sum = data_sum.astype({"id": int})
    for col in data_sum.columns[5:]:
        print(
            f"{col.upper():<13}: {data_sum[col].mean():.5f} [mean] {data_sum[col].max():.5f} [max] {data_sum[col].min():.5f} [min]"
        )
...
def statistic(
    named_data: dict[str, pd.DataFrame], column: str, *, start=2, stop=10, step=0.5
):
    ...
    for i in range(1, len(score_range)):
        ...
        for col in dd:
            if col == "score":
                continue
            dd[col].append(
                len(named_data[col][named_data[col][column] <= score_range[i]])
                - sum(dd[col])
            )
...
def plot_trainning(
    state_path: str,
    labels: list[str],
    highlight_on: str = "min",
    *,
    savefig: bool = False,
    figdir="./",
    title: str = "summary",
    vertical_line_at: list[int] = None,
    cut_from: int = 0,
):
    states = pd.read_csv(state_path, header=None)[cut_from:]
    ...
    for col in labels:
        plt.plot(range(cut_from + 1, N + cut_from + 1), states[col], label=col)
        plt.axhline(_values[col], linestyle="--")
        print(
            f"{highlight_on.upper()} [{col.rjust(_mm)}] = {_values[col]:.4f} at {_indexes[col] + 1}"
        )
    plt.title(title)
    plt.legend()
